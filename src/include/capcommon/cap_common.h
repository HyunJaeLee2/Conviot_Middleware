...
#define ERRASSIGNGOTO(res, err, label) {res=err; fprintf(stderr, "error! %d (%s:%d, strerror: %s, pid: %d)\n", res, __FILE__,__LINE__, strerror(errno), getpid()); goto label;}
#define IFVARERRASSIGNGOTO(var, val, res, err, label) if((var)==(val)) {res=err;fprintf(stderr, "error! (%s:%d, strerror: %s)\n", __FILE__,__LINE__, strerror(errno));goto label;}
#define ERRREADGOTO(read, read_full, res, label) if((read) < (read_full)) {res=ERR_CAP_READ_DATA_LACK;fprintf(stderr, "read error! (%s:%d, strerror: %s)\n", __FILE__,__LINE__, strerror(errno));goto label;}
#else
#define dp(fmt,args...)
#define dlp(fmt,args...)
#define print_id(comment)
#define ERRIFGOTO(res, label) if((res)!=ERR_CAP_NOERROR) {goto label;}
...
#define ERRREADGOTO(read, read_full, res, label) if((read) < (read_full)) {res=ERR_CAP_READ_DATA_LACK;goto label;}
#endif

#define CAPASSIGNGOTO(res, err, label) {res=err; goto label;}

#define ERRMEMGOTO(var, res, label) if((var)==NULL) {res=ERR_CAP_OUT_OF_MEMORY;goto label;}

#define SAFEMEMFREE(mem) if((mem) != NULL){free((mem));mem=NULL;}
